...
(* --we do not import Eqdep because it imports nasty hints automatically;
    we only [Require] it (no [Import]) so we can refer to [inj_pair2] by its
    fully-qualified name without polluting the hint databases. *)
From Stdlib Require Eqdep.

(* [inj_pair2] is derivable (it follows from [eq_rect_eq], the standard axiom
   already underlying [dependent induction]); we prove it here rather than
   postulating it, so this development declares no axioms of its own. *)
Lemma inj_pair2 :
  forall (U : Type) (P : U -> Type) (p : U) (x y : P p),
  existT P p x = existT P p y -> x = y.
Proof. intros. apply Eqdep.EqdepTheory.inj_pair2 with (1 := H). Qed.
...
(* The original LibTactics implemented [skip_with_axiom] using a postulated
   [skip_axiom : False] -- an outright soundness hole. This axiom-free
   development drops that axiom and routes [skip_with_axiom] through the
   existential-variable implementation instead: it lets a development proceed
   past a goal, but cannot close a proof unsoundly, since any leftover evar is
   rejected at [Qed]. *)
